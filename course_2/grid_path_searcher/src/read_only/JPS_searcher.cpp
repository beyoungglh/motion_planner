...
inline void JPSPathFinder::JPSGetSucc(GridNodePtr currentPtr, vector<GridNodePtr> & neighborPtrSets, vector<double> & edgeCostSets)
{
    ...
    for( int dev = 0; dev < num_neib + num_fneib; ++dev)
    {
        ...
        if( dev < num_neib )
        {
            ...
            if( !jump(currentPtr->index, expandDir, neighborIdx) )  //判断是否遇到强制邻点 终点等
                continue;
        }
        else
        {
    ...
    */
    ...
    while ( !openSet.empty() ){
        ...
        */
        ...
        currentPtr->id = -1; //add to closed list
...
//        ROS_WARN("open size %d", neighborPtrSets.size());
        /*
        *
        *
        ...
        */         
        for(int i = 0; i < (int)neighborPtrSets.size(); i++){
            ...
            */
            ...
            else if(tentative_gScore <= neighborPtr-> gScore)//已经在openlist通过current到达neighbor代价更低
